...
void UniformGrid::calculateGrid(unsigned int p_numTriangles, std::vector<RTMesh>* p_mesh, std::vector<RTMaterial>* p_material, Vector3 p_numVoxels){
	...
	std::vector<RTTriangle*>* aux_grid = new std::vector<RTTriangle*>[p_numVoxels.x * p_numVoxels.y * p_numVoxels.z];
	unsigned int size = 0;
	...
	m_gridArray = new GLfloat[p_numVoxels.x * p_numVoxels.y * p_numVoxels.z];
	m_triangleListArray = new GLuint[size];
	m_triangleVertexArray = new GLfloat[p_numTriangles * 3.0 * 3.0];
	m_triangleAmbientArray = new GLfloat[p_numTriangles * 3.0];
	m_triangleDiffuseArray = new GLfloat[p_numTriangles * 3.0];
	m_triangleSpecularArray = new GLfloat[p_numTriangles * 4.0];

	unsigned int gridCount = 0;
	for(int i=0; i<p_numVoxels.x * p_numVoxels.y * p_numVoxels.z; i++){
		for(int j=0; j<aux_grid[i].size(); j++){
			unsigned int triangleIndex = aux_grid[i].at(j)->index;
			m_triangleListArray[gridCount+j] = aux_grid[i].at(j)->index;
			...
			m_triangleSpecularArray[triangleIndex] = p_material->at(aux_grid[i].at(j)->materialIndex).mSpecular.r;
			m_triangleSpecularArray[triangleIndex+1] = p_material->at(aux_grid[i].at(j)->materialIndex).mSpecular.g;
			m_triangleSpecularArray[triangleIndex+2] = p_material->at(aux_grid[i].at(j)->materialIndex).mSpecular.b;
			m_triangleSpecularArray[triangleIndex+3] = p_material->at(aux_grid[i].at(j)->materialIndex).mSpecularExp;

			m_triangleDiffuseArray[triangleIndex] = p_material->at(aux_grid[i].at(j)->materialIndex).mDiffuse.r;
			m_triangleDiffuseArray[triangleIndex+1] = p_material->at(aux_grid[i].at(j)->materialIndex).mDiffuse.g;
			m_triangleDiffuseArray[triangleIndex+2] = p_material->at(aux_grid[i].at(j)->materialIndex).mDiffuse.b;
		}
